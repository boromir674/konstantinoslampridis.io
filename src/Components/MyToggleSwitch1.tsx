...
function ToggleSlider({
  ...
  const [progress, setProgressState] = useState(
    (active && !flip) || (!active && flip) ? 1 : 0
  );
  const [currentBarColor, setCurrentBarColor] = useState(
    active ? barBackgroundColorActive : barBackgroundColor
  );
  ...
  function setProgress(p: number) {
    const paddingLeft = Math.max(
      padding,
      Math.min(barWidth - padding - handleSize, p * barWidth - handleSize / 2)
    );
    const paddingProgress =
      (paddingLeft - padding) / (barWidth - padding * 2 - handleSize);
    setProgressState(p);
  ...
  handleStyles = active
    ? { ...handleStyles, ...handleStylesActive }
    : handleStyles;
  ...
  const bar = barRenderer ?? (
    <ToggleSliderBar
      ...
      transition={`all ${
        dragging ? "0s" : barTransitionDuration ?? transitionDuration
      }`}
      {...barStyles}
    />
  );
  const handle = handleRenderer ?? (
    <ToggleSliderHandle
      size={handleSize}
      ...
      transition={`all ${
        dragging ? "0s" : handleTransitionDuration ?? transitionDuration
      }`}
      {...handleStyles}
    />
  );
  ...
  return (
    ...
    >
      <div
        style={{
          paddingTop: Math.max(handleSize - barHeight, 0) / 2,
          ...overlayStyles,
        }}
      >
        ...
        <div
          style={{
            ...
            paddingLeft: Math.max(
              padding,
              Math.min(
                barWidth - padding - handleSize,
                progress * barWidth - handleSize / 2
              )
            ),
          }}
        >
